...
local function formatDate(dt)
  ...
  if dt.month < 1 or dt.month > 12 then error("Invalid month: " .. dt.month) end
  if dt.day < 1 or dt.day > 31 then error("Invalid day: " .. dt.day) end

  -- 4. Check for Time Components
  local has_time = (type(dt.hour) == "number") and (type(dt.min) == "number") and (type(dt.sec) == "number")
  ...
  if not has_time then
    ...
    if dt.hour < 0 or dt.hour > 23 then error("Invalid hour: " .. dt.hour) end
    if dt.min < 0 or dt.min > 59 then error("Invalid min: " .. dt.min) end
    if dt.sec < 0 or dt.sec > 61 then error("Invalid sec: " .. dt.sec) end
