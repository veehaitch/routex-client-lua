...
context("VCEK parsing", function ()
  ...
  test("Rejects invalid SEV-VCEK", function ()
    ...
    assert.error_match(function () sevVcek:verify() end, "Expected issuer SEV-Genoa, got: SEV-Turin", nil, true)
  end)
  ...
  test("Rejects invalid SEV-VCEK signature", function ()
    ...
    assert.error_match(
      function () sevVcek:verify() end,
      "Failed to verify certificate SEV-VCEK with issuer SEV-Genoa: invalid signature",
      nil,
      true
    )
  end)
