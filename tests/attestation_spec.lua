...
context("Remote attestation", function ()
  test("Works with valid SEV-VCEK and attestation report (Genoa v5)", function ()
    ...
    assert.has.no.error(
      function () verify(attestation) end
    )
  end)
  ...
  test("Rejects valid SEV-VCEK with unmet requirements (Genoa v3)", function ()
    ...
    assert.error_match(
        function () verify(attestation) end,
        "Report's committed version does not fulfill requirements. Expected at least 1.55.49, got 1.55.39",
        nil,
        true
      )
  end)
  ...
  test("Rejects valid SEV-VCEK with unmet requirements (Milan)", function ()
    ...
    assert.error_match(
      function () verify(attestation) end,
      "Report's committed version does not fulfill requirements. Expected at least 1.55.35, got 1.55.29",
      nil,
      true
    )
  end)
  ...
  test("Rejects valid SEV-VCEK with unmet requirements (Turin)", function ()
    ...
    assert.error_match(
      function () verify(attestation) end,
      "Report's committed version does not fulfill requirements. Expected at least 1.55.65, got 1.55.61",
      nil,
      true
    )
  end)
  ...
  test("Rejects invalid attestation report", function ()
    ...
    assert(reportBytes ~= nil)
    reportBytes =
      reportBytes:sub(1, Offsets.base.signature) ..
      ("W"):rep(512) ..
    ...
    assert.error_match(
      function () verify(attestation) end,
      "Failed to verify attestation report with SEV-VCEK public key",
      nil,
      true
    )
  end)
  ...
  context("Requirements", function ()
    ...
    test("Accepts higher committed version", function ()
      ...
      assert.no_error(function () verify(attestation) end)
    end)
    ...
    test("Rejects invalid committed SNP TCB version", function ()
      ...
      assert.error_match(
        function () verify(attestation) end,
        "Expected minimum committed SNP TCB of 27, got 26",
        nil,
        true
      )
    end)
    ...
    test("Rejects incomplete alias check", function ()
      ...
      assert.error_match(
        function () verify(attestation) end,
        "Expected alias check to have been completed",
        nil,
        true
      )
    end)
    ...
    test("Rejects debug-enabled guest policy", function ()
      ...
      assert.error_match(
        function () verify(attestation) end,
        "Debug mode is enabled in guest policy",
        nil,
        true
      )
    end)

    test("Rejects non-zero VMPL", function ()
      local attestation = RemoteAttestationStubbed:new(vcekPem, reportBytes)
      attestation.report.vmpl = 1
      assert.error_match(
        function () verify(attestation) end,
        "Expected VMPL 0, got 1",
        nil,
        true
      )
    end)
    ...
    test("Rejects invalid committed microcode version", function ()
      local attestation = RemoteAttestationStubbed:new(vcekPem, reportBytes)
      attestation.requirements.minMicrocode = function (_, _) return attestation.report.reportedTcb.microcode + 1 end

      assert.error_match(
        function () verify(attestation) end,
        "Expected minimum microcode version of 89, got 88",
        nil,
        true
      )
    end)
